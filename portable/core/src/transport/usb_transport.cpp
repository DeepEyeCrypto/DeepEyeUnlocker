...
bool LibUsbTransport::Open(int fd) {
  ...
  int rc = libusb_wrap_sys_device(
      reinterpret_cast<libusb_context*>(_ctx), 
      (intptr_t)fd, 
...
#ifdef HAS_LIBUSB
  if (_handle) {
    libusb_release_interface(reinterpret_cast<libusb_device_handle*>(_handle), 0);
...
int LibUsbTransport::Send(const uint8_t *data, size_t length, uint32_t timeout_ms) {
#ifdef HAS_LIBUSB
  int transferred = 0;
...
int LibUsbTransport::Receive(uint8_t *data, size_t length, uint32_t timeout_ms) {
#ifdef HAS_LIBUSB
  int transferred = 0;
  libusb_bulk_transfer(
      reinterpret_cast<libusb_device_handle*>(_handle), 
