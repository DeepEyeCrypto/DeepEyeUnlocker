            ...
            if vid == v && pid == p {
                ...
                let read_str = |idx: u8| -> Option<String> {
                    ...
                    handle
                        .as_ref()
                        .zip(lang)
                        .and_then(|(h, l)| h.read_string_descriptor(l, idx, timeout).ok())
                };
...
pub async fn run_mtk_brom_bypass(app: AppHandle) -> Result<String, String> {
    app.emit("log", "🔵 MTK BROM Bypass — scanning USB for MTK device...")
        .ok();

    // Step 1: detect device
    let device = mtk_brom::mtk_detect_device()
        .await
    ...
    let chip = mtk_brom::mtk_handshake_and_identify()
        .await
        .map_err(|e| format!("❌ Handshake failed: {e}"))?;
    app.emit(
        "log",
        &format!("✅ Chip: {} (HW: 0x{:04X})", chip.chip_name, chip.hw_code),
    )
    .ok();

    // Step 3: SLA bypass
    app.emit("log", "🔓 Attempting SLA bypass...").ok();
    mtk_brom::mtk_bypass_sla()
        .await
...
pub async fn run_da_bypass(app: AppHandle) -> Result<String, String> {
    ...
    let device = mtk_brom::mtk_detect_device()
        .await
        .map_err(|e| format!("❌ {e}"))?;
    app.emit("log", &format!("✅ Found {:?} device", device.mode))
        .ok();

    let chip = mtk_brom::mtk_handshake_and_identify()
        .await
        .map_err(|e| format!("❌ {e}"))?;
    app.emit("log", &format!("✅ Chip: {}", chip.chip_name))
        .ok();

    let auth = mtk_brom::mtk_detect_auth_type()
        .await
        .map_err(|e| format!("❌ {e}"))?;
...
pub async fn run_meta_bypass(app: AppHandle) -> Result<String, String> {
    app.emit("log", "🔵 MTK META Mode — detecting device...")
        .ok();

    let device = mtk_brom::mtk_detect_device()
        .await
        .map_err(|e| format!("❌ {e}"))?;
    app.emit("log", &format!("✅ Device: {:?}", device.mode))
        .ok();
    ...
    app.emit(
        "log",
        &format!(
            "📋 Chip: {} | HW sub: 0x{:04X} | SW: 0x{:04X}",
            chip.chip_name, chip.hw_sub_code, chip.sw_ver
        ),
    )
    .ok();
...
pub async fn run_adb_frp(app: AppHandle) -> Result<String, String> {
    app.emit("log", "🔵 ADB FRP Bypass — listing devices...")
        .ok();

    let devices = adb::adb_list_devices(app.clone())
        .await
        .map_err(|e| format!("❌ {e}"))?;

    if devices.is_empty() {
        return Err(
            "❌ No ADB devices found. Connect device with USB debugging enabled.".to_string(),
        );
    }
    ...
    app.emit(
        "log",
        &format!("✅ Target: {} ({})", serial, devices[0].model),
    )
    .ok();

    // Method 1: Settings provider
    app.emit("log", "  🔧 Method 1: Clearing FRP via settings...")
        .ok();
    adb::adb_shell_command(
        app.clone(),
        serial.clone(),
        "settings put global device_provisioned 1".to_string(),
    )
    .await
    .ok();
    adb::adb_shell_command(
        app.clone(),
        serial.clone(),
        "settings put secure user_setup_complete 1".to_string(),
    )
    .await
    .ok();
    adb::adb_shell_command(
    ...
    // Method 2: dd wipe
    app.emit("log", "  🔧 Method 2: FRP partition wipe via dd...")
        .ok();
    adb::adb_erase_frp_partition(app.clone(), serial.clone())
        .await
        .ok();

    // Method 3: pm clear fallback
    app.emit("log", "  🔧 Method 3: Package manager clear...")
        .ok();
    adb::adb_shell_command(
        app.clone(),
        serial.clone(),
        "pm clear com.google.android.gms".to_string(),
    )
    .await
    .ok();

    app.emit("log", "🔄 Rebooting device...").ok();
    adb::adb_reboot_device(app.clone(), serial.clone(), "system".to_string())
        .await
        .ok();
...
pub async fn run_deepeye_agent(app: AppHandle) -> Result<String, String> {
    app.emit("log", "🔵 DeepEye Agent — listing ADB devices...")
        .ok();
    ...
    // Check root access
    let is_root = adb::adb_check_root_access(app.clone(), serial.clone())
        .await
        .unwrap_or(false);
    app.emit(
        "log",
        &format!(
            "🔑 Root: {}",
            if is_root { "YES" } else { "NO (limited mode)" }
        ),
    )
    .ok();

    // Get full device info
    let info = adb::adb_get_full_info(app.clone(), serial.clone())
        .await
        .map_err(|e| format!("❌ {e}"))?;
    app.emit(
        "log",
        &format!(
            "📱 {} {} | Android {} | BL: {} | FRP: {}",
            info.brand, info.model, info.android_version, info.bootloader_status, info.frp_status
        ),
    )
    .ok();
...
pub async fn run_pattern_bypass(app: AppHandle) -> Result<String, String> {
    ...
    if devices.is_empty() {
        return Err(
            "❌ No ADB device found. Boot to Recovery or connect with root ADB.".to_string(),
        );
    }
    ...
    for file in &lock_files {
        ...
        adb::adb_shell_command(app.clone(), serial.clone(), cmd)
            .await
            .ok();
    }
    ...
    adb::adb_shell_command(app.clone(), serial.clone(), sqlite_cmd.to_string())
        .await
        .ok();
...
pub async fn run_qcom_edl(app: AppHandle) -> Result<String, String> {
    app.emit("log", "⚡ Qualcomm EDL — scanning for 9008 device...")
        .ok();

    let device = edl::edl_find_device()
        .await
        .map_err(|e| format!("❌ {e}"))?;
    app.emit(
        "log",
    ...
    let sahara = edl::edl_sahara_handshake()
        .await
        .map_err(|e| format!("❌ Sahara failed: {e}"))?;
    app.emit(
        "log",
        &format!(
            "✅ Sahara v{} mode={} maxPkt={}",
            sahara.version, sahara.mode, sahara.max_packet_size
        ),
    )
    .ok();
...
pub async fn run_qcom_frp_erase(app: AppHandle) -> Result<String, String> {
    app.emit("log", "⚡ Qualcomm FRP Erase via Firehose...")
        .ok();

    // Erase FRP partition
    edl::edl_erase_partition("frp".to_string())
        .await
        .map_err(|e| format!("❌ FRP erase failed: {e}"))?;
    app.emit("log", "✅ FRP partition erased via Firehose!")
        .ok();
...
pub async fn run_sahara_handshake(app: AppHandle) -> Result<String, String> {
    ...
    let info = edl::edl_sahara_handshake()
        .await
        .map_err(|e| format!("❌ {e}"))?;
    ...
    app.emit(
        "log",
        &format!(
            "✅ Sahara v{} | Mode: {} | MaxPkt: {}B",
            info.version, info.mode, info.max_packet_size
        ),
    )
    .ok();
...
pub async fn get_ios_device_info(app: AppHandle) -> Result<String, String> {
    ...
    let info = run_binary(&app, "ideviceinfo", &["-u", &udid]).await?;
...
#[command]
pub async fn check_activation_status(app: AppHandle) -> Result<String, String> {
    let udid = get_connected_udid(&app).await?;

    // Check ActivationState via ideviceinfo
    let info = run_binary(&app, "ideviceinfo", &["-u", &udid, "-k", "ActivationState"]).await?;
...
#[command]
pub async fn run_activation_bypass(app: AppHandle) -> Result<String, String> {
    // Step 1: Get UDID
    let udid = get_connected_udid(&app).await?;

    // Step 2: Check current activation state
    let state = run_binary(&app, "ideviceinfo", &["-u", &udid, "-k", "ActivationState"])
        .await
        .unwrap_or_default();
    ...
    let result = run_binary(&app, "ideviceactivation", &["-u", &udid, "state"]).await;
    ...
    match result {
        Ok(out) => {
            ...
            if out.contains("Unactivated") || out.contains("NotActivated") {
                // Try activation
                let act_result =
                    run_binary(&app, "ideviceactivation", &["-u", &udid, "activate"]).await;
...
// ── MDM Profile Bypass ──────────────────────────────
#[command]
pub async fn run_mdm_bypass(app: AppHandle) -> Result<String, String> {
    let udid = get_connected_udid(&app).await?;

    // List profiles first
    let profiles = run_binary(&app, "ideviceprovision", &["list", "-u", &udid])
        .await
        .unwrap_or_else(|_| "Could not list profiles".to_string());

    // Remove all MDM profiles
    let remove = run_binary(&app, "ideviceprovision", &["remove-all", "-u", &udid]).await;
...
// ── Force DFU Mode ──────────────────────────────────
#[command]
pub async fn run_force_dfu(app: AppHandle) -> Result<String, String> {
    let udid = get_connected_udid(&app).await?;

    if udid.is_empty() {
        return Err("No iOS device found.\n\
             Manual DFU: \n\
             iPhone 8+: Vol Down → Vol Up → Hold Side 10s\n\
             iPhone 7: Hold Vol Down + Power 10s\n\
             iPhone 6s: Hold Home + Power 10s"
            .to_string());
    }

    // Use idevicediagnostics to restart into recovery
    let result = run_binary(&app, "idevicediagnostics", &["restart", "-u", &udid]).await;
...
#[command]
pub async fn run_passcode_remove(app: AppHandle) -> Result<String, String> {
...
pub async fn run_shsh_save(app: AppHandle) -> Result<String, String> {
    ...
    let ecid_raw = run_binary(&app, "ideviceinfo", &["-u", &udid, "-k", "UniqueChipID"]).await?;
    let ecid = ecid_raw.trim().to_string();

    let model = run_binary(&app, "ideviceinfo", &["-u", &udid, "-k", "ProductType"])
        .await
        .unwrap_or_else(|_| "unknown".to_string());
    let model = model.trim().to_string();

    let ios_ver = run_binary(&app, "ideviceinfo", &["-u", &udid, "-k", "ProductVersion"])
        .await
        .unwrap_or_default();
    let ios_ver = ios_ver.trim().to_string();

    // Try tsschecker or blobsaver
    let tsschecker = run_binary(
        &app,
        "tsschecker",
        &[
            "-d",
...
// ── IPSW Flash ──────────────────────────────────────
#[command]
pub async fn run_ipsw_flash(app: AppHandle, ipsw_path: String) -> Result<String, String> {
    if ipsw_path.is_empty() {
        return Err("No IPSW path provided. \
            Select .ipsw file first."
            .to_string());
    }
    ...
    let result = run_binary(&app, "idevicerestore", &["-u", &udid, &ipsw_path]).await;
...
#[command]
pub async fn run_samsung_frp(app: AppHandle, serial: String) -> Result<String, String> {
    // Method 0: Auth Check
...
#[command]
pub async fn run_samsung_odin_info(app: AppHandle) -> Result<String, String> {
    // Check if heimdall is available (open-source Odin alternative)
    let heimdall = run_binary(&app, "heimdall", &["version"]).await;

    match heimdall {
        Ok(ver) => {
            let devices = run_binary(&app, "heimdall", &["detect"])
                .await
                .unwrap_or_else(|_| "No Samsung device in Download mode".to_string());
            Ok(format!(
                "✅ Heimdall {ver}\nDetected: {devices}\n\n\
...
const TESTPOINT_DB: &[(&str, &str, &str, &str, &str, &str)] = &[
    (
        ...
        "MT6789",
        "brom_pin",
        "Short TP4 to GND while powering on",
        "Near battery connector — TP4",
        "Hold Vol- + connect USB after shorting",
    ),
    (
        "Redmi 9 (M2004J19G)",
        "MT6769",
        "brom_pin",
        "Short TP19 to ground",
        "Back of PCB near NFC coil",
        "Use copper wire, brief contact",
    ),
    (
        "Samsung A12 (SM-A125)",
        "MT6765",
        "brom_pin",
        "Short TP1 to GND on motherboard",
        "Near USB-C port — marked TP1",
        "Remove battery before shorting",
    ),
    (
        "Poco M2 (MZB07ZIIN)",
        "MT6768",
        "brom_pin",
        "Short BOOT0 pad to GND",
        "Left side of CPU — small via",
        "Works best with tweezers",
    ),
    (
        "Realme 14x (RMX3645)",
        "MT6769",
        "brom_pin",
        "Short CLK to GND on eMMC",
        "Under RF shield — follow CLK trace",
    ...
    (
        ...
        "Exynos 850",
        "uart",
        "UART test point near CPU",
        "Bottom PCB — labeled UART TX",
        "Connect UART adapter at 115200",
    ),
    (
        "OnePlus 9 (LE2115)",
        "SM8350",
        "voltage",
        "EDL via Vol+ + Power hold 15s",
        "No physical TP needed",
        "Enter EDL via key combo",
    ),
    (
        "iPhone X (A1901)",
        "A11 Bionic",
        "dfu",
        "checkm8 exploit — PP_CPU_VCORE",
        "Near CPU — requires microscope",
        "A11 supported by checkm8",
    ),
    (
        "Nokia G21 (TA-1418)",
        "Unisoc T606",
        "brom_pin",
        "Short eMMC DAT0 to GND",
        "Above PMIC — small via",
        "Use 0.1mm wire",
    ),
    (
        ...
        "MT6761",
        "brom_pin",
        "Short BOOT pin near PMIC",
        "Marked TP on PCB silkscreen",
        "Remove back cover + midframe",
    ),
    (
        "Redmi Note 10 (Sunny)",
        "SDM678",
        "edl_pin",
        "Short 2 TP below eMMC",
        "Near shielding - top right",
        "Fastboot to EDL possible via cable",
    ),
    (
        "Xiaomi 11T (Amber)",
        "MT6893",
        ...
        "Short CLK to GND",
        "Under CPU shield",
        "Requires heating to open back",
    ),
    (
        "Samsung S22 (SM-S901)",
        "Snapdragon 8 Gen 1",
        "edl_pin",
        "Short 2 golden pads near battery",
        "Right of charging flex",
        "Use EDL cable for better success",
    ),
    (
        "Oppo Reno 6 (CPH2251)",
        "MT6877",
        "brom_pin",
        "Short VCORE to GND",
        "Bottom PCB near flex",
        "Easy access without shield removal",
    ),
    (
        "Vivo Y20 (V2027)",
        "SDM460",
        "edl_pin",
        "Short 2 testpoints near SIM slot",
        "Under metal plate",
        "Force EDL via power button + TP",
    ),
    (
        "Realme GT Neo 2",
        "Snapdragon 870",
        "edl_pin",
        "Short pads near USB connector",
        "Exposed pads - no shielding",
        "Use 9008 driver",
    ),
    (
        "Redmi Note 11 (Spes)",
    ...
    (
        ...
        "MT6769",
        "brom_pin",
        "Short TP pads near LCD flex",
        "Left side of battery",
        "Use BROM handshake mode",
    ),
    (
        "Xiaomi Mi 11 Lite",
        "Snapdragon 732G",
        "edl_pin",
        "Short 2 pins near volume buttons",
        "Internal ribbon layer",
        "Remove screws and mid-frame",
    ),
    (
        "Oppo A54 (CPH2239)",
        "MT6765",
        "brom_pin",
        "Short TP pads near camera",
        "Top right corner",
        "Standard BROM bypass",
    ),
    (
        "Vivo V21 (V2050)",
        "MT6853",
        "brom_pin",
        "Short CLK pad near RAM",
        "Exposed via small hole",
        "Use 1.8V logic",
    ),
    (
        "Realme 8 (RMX3085)",
    ...
    (
        ...
        "MT6769",
        "brom_pin",
        "Short pads near speaker",
        "Bottom right",
        "Standard MediaTek protocol",
    ),
    (
        "Samsung A52 (SM-A525)",
        "Snapdragon 720G",
        "edl_pin",
        "Short 2 pins near NFC",
        "Center of motherboard",
        "Use EDL cable + key combo",
    ),
    (
        "Xiaomi Mi 10T",
        "Snapdragon 865",
        "edl_pin",
        "Short pins near 5G antenna",
        "Right side edge",
        "High-speed EDL protocol",
    ),
    (
        "Oppo F19 (CPH2219)",
        "Snapdragon 662",
        "edl_pin",
        "Short pins near fingerprint",
        "Internal back plate",
    ...
    (
        ...
        "MT6762",
        "brom_pin",
        "Short TP pads near SD slot",
        "Inside SIM tray area",
        "No full disassembly needed",
    ),
    (
        "Realme Narzo 50",
        "MT6781",
        "brom_pin",
        "Short pads near PMIC",
        "Top PCB area",
        "Standard BROM tools",
    ),
    (
...
pub fn search_testpoints(query: String) -> Vec<TestpointInfo> {
    ...
    TESTPOINT_DB
        ...
        .map(
            |(device, chipset, method, desc, pin, notes)| TestpointInfo {
...
pub async fn get_connected_udid(app: &AppHandle) -> Result<String, String> {
    let raw = run_binary(app, "idevice_id", &["-l"]).await.map_err(|e| {
        format!(
    ...
    let udid = raw
        ...
        .ok_or_else(|| {
            "❌ No iOS device found.\n\
             ...
             • iPhone unlocked\n\
             • Settings > Developer Mode ON (iOS 16+)"
                .to_string()
        })?;
...
pub async fn ensure_adb_authorized(app: &AppHandle, serial: &str) -> Result<(), String> {
    let devices = run_binary(app, "adb", &["devices"]).await.map_err(|e| {
        format!(
            "❌ ADB not found: {e}\n\
...
#[command]
pub async fn check_usb_permissions(app: AppHandle) -> Result<String, String> {
    // On macOS check SIP and USB access
    let sip = run_binary(&app, "csrutil", &["status"])
        .await
...
#[command]
pub async fn restart_adb_server(app: AppHandle) -> Result<String, String> {
...
#[command]
pub async fn check_samsung_download_mode(app: AppHandle) -> Result<String, String> {
    ...
    match result {
        ...
        Ok(_) => Err("❌ No Samsung device in Download mode.\n\
             💡 Enter Download mode:\n\
...
#[command]
pub async fn run_full_bypass(app: AppHandle, platform: String) -> Result<String, String> {
    match platform.to_uppercase().as_str() {
        ...
        "SAMSUNG" => {
            ...
            // Get serial first
            let serial = run_binary(&app, "adb", &["get-serialno"])
                .await
                .unwrap_or("".into());
        ...
        _ => Err(format!(
            "❌ Platform '{}' not supported for one-click bypass",
            platform
        )),
    }
}
