...
#[tauri::command]
pub async fn diag_test_handshake(app: AppHandle, protocol: String) -> Result<HandshakeResult, String> {
    ...
    match protocol.as_str() {
        ...
        "Edl" => {
            match edl::edl_find_device().await {
                Ok(info) => {
                    Ok(HandshakeResult {
        ...
        _ => Err(format!("Protocol {} not supported for diagnostics", protocol)),
    }
}
