...
namespace DeepEyeUnlocker.Tests.Protocols.Fuzzing
{
    ...
    public class FirehoseFuzzingTests
    {
        private readonly string _scenarioDir;
        private readonly ITestOutputHelper _output;

        public FirehoseFuzzingTests(ITestOutputHelper output)
        {
            _output = output;
        ...
        [Fact]
        public async Task Firehose_FuzzConfig_StructuredCampaign()
        {
            // Arrange
            ...
            // Act
            var crashes = await harness.FuzzFirehoseAsync(scenario, iterations);

            // Assert & Report
            string reportPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "fuzz_reports", "firehose_fuzz_report.json");
            ...
            {
                string crashPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "fuzz_findings", $"firehose_crash_seed_{crash.Seed}.json");
